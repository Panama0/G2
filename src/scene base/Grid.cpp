...
void Grid::init(sf::Vector2u renderSpace, sf::Vector2u squareSize)
{
    ...
    if(!m_gridTexture.resize(renderSpace)) { std::cerr << "Could not resize!\n"; }
...
void Grid::drawToTexture()
{
    ...
    for(size_t i {}; i < m_gridSize.x + 1; i++)
    {
        ...
        sf::Vertex a {{static_cast<float>(m_squareSize.x * i - thickness / 2.f), 0.f}, col}; 
    ...
    for(size_t i {}; i < m_gridSize.y; i++)
    {
        ...
        sf::Vertex a {{0.f, static_cast<float>(m_squareSize.y * i - thickness / 2.f)}, col};
